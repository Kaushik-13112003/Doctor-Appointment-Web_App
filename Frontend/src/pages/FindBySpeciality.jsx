      ...
      <div className="flex flex-col gap-6 mt-8 items-center">
        ...
        <div className="flex justify-center items-center  flex-wrap gap-8 w-[100%]">
          {specialtyData.map((ele) => (
            <NavLink
              to={`/${ele?.specialty ? ele?.specialty : "all-doctors"}`}
              key={ele.id}
              className="flex  flex-col gap-3 items-center "
            >
              <img
                src={ele.image ? ele.image : "./banner.png"}
                alt={ele.specialty}
                className="w-[100px]  border border-gray-500 hover:shadow-[2px_2px_blue] hover:cursor-pointer duration-500 h-[100px] rounded-full"
              />
              <h1>{ele.specialty}</h1>
            </NavLink>
          ))}
